...
## Обновление: слоты с промежутком времени

В разделе `/admin` → **Запись** теперь слот создаётся как интервал:
...
Пример: `08.05.2026`, `10:00`, `11:00`, `1 место`.

Клиент на главной странице увидит не просто `10:00`, а `10:00–11:00`.

Если проект уже был подключён к Supabase, заново выполните файл `supabase.sql` в **Supabase → SQL Editor**. Он безопасно добавит колонку `end_time` и обновит view `available_slots`.

## Фото на главной странице

Для аватарки и слайдера используется публичный bucket Supabase Storage:
...
Для одинаковой вкладки «Клиенты» на телефоне и компьютере запусти файл `clients-sync.sql` в Supabase SQL Editor.
После обновления сайта клиенты и их тренировки будут храниться в Supabase, а не только в памяти браузера.
